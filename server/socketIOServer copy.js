...
const SocketIOServer = () => {
  ...
  const setEventHandlers = (socket) => {
    ...
    updateDate(socket);    
    libraryRoom.init(socket);
    restRoom.init(socket);
  }
  ...
  */
  ...
  const onRequestChangeScene = (socket, request) => {
    ...
    switch (prevScene) {
      case "Home": ; break;
      case "Library": libraryRoom.remove(id); break;
      case "Study": ; break;
      case "Rest": restRoom.remove(id); break;
    }

    switch (currentScene) {
      case "Home": ; break;
      case "Library": libraryRoom.add(id); break;
      case "Study": ; break;
      case "Rest": restRoom.add(id); break;
    }
